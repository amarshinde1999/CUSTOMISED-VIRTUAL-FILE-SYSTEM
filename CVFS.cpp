 ...
 typedef struct filetable
{
 ...
 void man(char *name)
 {
   ...
   else if(strcmp(name,"write")==0)
   {
	 ...
	 printf("Usage:write File_name\n After this enter the datathat we want to write\n");
   }
   ...
   {
    printf("Description:Used to open Existng File\n");
	...
	else if(strcmp(name,"lseek")==0)
	{
	  ...
	  printf("Usage:lseek File_Name changeInOffset StartPont\n");
	}
...
void DisplayHelp()
{
 ...
 printf("close:To close the the file\n");
 printf("Close all: To close all opened All files\n");
  ...
  int rm_File(char *name)
{
 ...
 {
	if(((UFDTArr[fd].ptrfiletable->mode)!=WRITE)&&((UFDTArr[fd].ptrfiletable->ptrinode->permission)!=READ+WRITE))    return -1;
...
int fstat_file(int fd)
{
 ...
 printf("\n---- Statstical Information about file-----\n");
 printf("File name:%s\n",temp->FileName);
 ...
 if(temp->permission==1)
   ...
   printf("File Permission:	Write\n");
    ...
    printf("FilePermission :READ&WRITE");
   printf("-------------------------\n\n");
   ...
   if(count==1)
	...
	else if(count==2)
	{
	  if(strcmp(command[0],"stat")==0)
	  {
		...
		else if(strcmp(command[0],"write")==0)
		{
		   ...
		   if(ret==-2)
		   printf("ERROR:There is no Sufficient Memory Write\n");
		   if(ret==-3)
	...
	else if(count==3)
	{
	    if(strcmp(command[0],"create")==0)
		{
		  ...
		  if(ret>=0)
		  printf("File is Successfully created with the descriptor: %d\n",ret);
		  if(ret==-1)
		  printf("ERROR:Incorrect Parameters\n");
		  if(ret==-2)
